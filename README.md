...
- **스키마 변경사항 추출**:
  - 특정 이슈 범위의 DDL 스크립트 추출 (diff)
  - Unix pipeline 지원으로 유연한 활용 가능
- **스키마 덤프**:
  - 특정 시점의 완전한 데이터베이스 스키마 덤프
  - 스키마 복원, 분석, 새 환경 구축에 활용
...
shelltide diff staging/bridge

# 특정 이슈 범위만 추출
shelltide diff staging/bridge --from 100 --to 105

# 특정 시작점부터 최신까지
shelltide diff staging/bridge --from 50

# 스크립트가 없으면 exit code 2로 종료 (자동화용)
shelltide diff staging/bridge --from 100 --to 105 --fail-if-empty
```

**출력 예시:**
```sql
-- Schema changes from issue #100 to #105
-- Generated by shelltide on 2025-09-10
...
Bytebase를 동기화한 현재 데이터베이스 스키마 또는 changelog에 저장된 특정 시점의 스키마를 덤프할 수 있습니다. 새 환경 구축이나 스키마 분석에 활용됩니다.

```sh
# Bytebase 동기화 후 현재 상태의 완전한 스키마 덤프
shelltide dump dev/chat

# 특정 이슈 시점의 스키마 덤프 (해당 이슈 이하 최신 마이그레이션)
shelltide dump dev/chat --at-issue 300

# 스키마가 없으면 exit code 2로 종료 (자동화용)
shelltide dump dev/chat --at-issue 100 --fail-if-empty
```

**출력 예시:**
```sql
-- Database schema dump at or before issue #300
-- Actual issue: #297
-- Migration executed: 2025-04-02T08:51:09Z
-- Generated by shelltide on 2025-09-10
...
**diff vs dump 차이점:**
- `diff`: 특정 범위의 변경사항 (migration scripts)
- `dump`: 특정 시점의 완전한 스키마 상태 (full schema)
